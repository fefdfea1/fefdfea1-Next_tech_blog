...
<p >
<img
  src="/mdxImg/DevelopPost/createPostList/blogDir.png"
  alt="현재 블로그 폴더구조"
  width="200"
...
<img
src="/mdxImg/DevelopPost/createPostList/folderDir.png"
alt="mdx 폴더 내부 구조"
width="240"
style={{ height: "300px" }}
/>
...
그리고 그 내부의 구조를 보면 mdxPost > DevelopPost > createPostList > mdx파일 이런 식으로 각 카테고리와 개별적인 폴더 , mdx파일로 이루어져 있습니다. 이렇게 만든 이유는 각 폴더에 필요한 정보들을 넣어 관리하기 편하고 카테고리별로 게시글을 분류 할 수 있어 이렇게 만들었지만 꼭 이렇게 만들 필요없이 원하는 방식이 있다면 자유롭게 만들 수도 있습니다. 저와 동일하게 구조를 만들고 싶은분들만 이렇게 하는걸 추천드립니다.
...
<img src="/mdxImg/DevelopPost/createPostList/mdxCode.png" alt="mdx 내부코드" />
...
```javascript
  ...
  category?: string,
  slug?: string,
  createPostList?: string
) => {
  const categoryName = category || "**";
  const fileName = slug || "**";
  const createPostListName = createPostList || "**";
  let postPaths = globSync(
    `${POSTS_PATH}/${categoryName}/${createPostListName}/${fileName}.mdx`
  );
  return postPaths;
};
...
위의 코드를 출력하면 아래와 같이 이 포스트에 대한 Front-Matter가 출력됩니다. 너무 길어서 이미지를 좀 잘랐지만 아해에 readingTime도 함께 나와있습니다.
<img src="/mdxImg/DevelopPost/createPostList/postData.png" alt="포스트 파싱 데이터"/>
